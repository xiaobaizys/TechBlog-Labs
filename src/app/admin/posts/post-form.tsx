...
export function PostForm({ initialData }: PostFormProps) {
  ...
  async function handleCoverUpload(e: React.ChangeEvent<HTMLInputElement>) {
    ...
    // 重置 input 否则连选同一张图不会触发 onChange
    e.target.value = ''
    if (!file) return
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    startTransition(async () => {
      try {
        const url = isEdit ? `/api/posts/admin/${initialData!.id}` : '/api/posts'
    ...
    <form onSubmit={handleSubmit} className="space-y-6" noValidate>
      ...
      <div>
        ...
        <div className="flex flex-wrap gap-2">
          ...
          <input ref={fileInputRef} type="file" accept="image/jpeg,image/png,image/webp,image/gif" onChange={handleCoverUpload} className="hidden" />
        </div>
        <p className="mt-1 text-xs text-[rgb(var(--muted-foreground))]">支持 jpg / png / webp / gif，单张不超过 5MB</p>
        ...
        {form.coverImage && (
          <div className="mt-3 overflow-hidden rounded-lg border border-[rgb(var(--border))] bg-[rgb(var(--muted))]">
            ...
            <div className="flex items-center justify-between gap-2 border-t border-[rgb(var(--border))] bg-[rgb(var(--card))] px-3 py-1.5 text-xs">
              ...
              <button type="button" onClick={() => updateField('coverImage', '')} className="shrink-0 text-[rgb(var(--muted-foreground))] hover:text-red-500">
                移除
              </button>
            </div>
          </div>
      ...
      <div>
        ...
        <div className="flex gap-2">
          <input
            ...
            className="flex-1 rounded-lg border border-[rgb(var(--border))] bg-[rgb(var(--card))] px-4 py-2.5 text-sm outline-none focus:border-primary-500 focus:ring-2 focus:ring-primary-500/20"
          />
          <button type="button" onClick={addTag} className="rounded-lg border border-[rgb(var(--border))] bg-[rgb(var(--card))] px-4 py-2.5 text-sm transition-colors hover:bg-[rgb(var(--muted))]">
            添加
          </button>
        </div>
        {form.tags.length > 0 && (
          <div className="mt-2 flex flex-wrap gap-1.5">
            {form.tags.map((tag) => (
              <span key={tag} className="inline-flex items-center gap-1 rounded-full border border-[rgb(var(--border))] bg-[rgb(var(--muted))] px-2.5 py-0.5 text-xs">
                {tag}
                <button type="button" onClick={() => removeTag(tag)} className="ml-0.5 text-[rgb(var(--muted-foreground))] hover:text-red-500">
                  ×
                </button>
              </span>
            ))}
      ...
      <div>
        ...
        />
        <p className="mt-1 text-xs text-[rgb(var(--muted-foreground))]">支持 Markdown 和 MDX 语法。代码块使用 ```language 包裹。</p>
      </div>
      ...
      <div className="flex flex-wrap items-center gap-6">
        <div>
          ...
          <select id="status" value={form.status} onChange={(e) => updateField('status', e.target.value)} className="rounded-lg border border-[rgb(var(--border))] bg-[rgb(var(--card))] px-4 py-2.5 text-sm outline-none focus:border-primary-500">
        ...
        <div className="flex items-center gap-2 pt-5">
          <input type="checkbox" id="featured" checked={form.featured} onChange={(e) => updateField('featured', e.target.checked)} className="h-4 w-4 rounded border-[rgb(var(--border))] text-primary-600 focus:ring-primary-500" />
      ...
      <div className="flex items-center gap-3 pt-4 border-t border-[rgb(var(--border))]">
        <button type="submit" disabled={isPending} className="btn-shimmer disabled:cursor-not-allowed disabled:opacity-60">
          {isPending ? '保存中...' : isEdit ? '更新文章' : '创建文章'}
        </button>
        <Link href="/admin/posts" className="rounded-lg border border-[rgb(var(--border))] bg-[rgb(var(--card))] px-6 py-2.5 text-sm font-medium transition-colors hover:bg-[rgb(var(--muted))]">
          取消
        </Link>
      </div>
    </form>
